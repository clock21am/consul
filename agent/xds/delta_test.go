...
func TestServer_DeltaAggregatedResources_v3_BasicProtocol_TCP(t *testing.T) {
	...
	runStep(t, "initial setup", func(t *testing.T) {
		...
		// cleaned up.
		envoy.SendDeltaReq(t, shared.ClusterType, &envoy_discovery_v3.DeltaDiscoveryRequest{
	...
	runStep(t, "first sync", func(t *testing.T) {
		...
		envoy.SendDeltaReq(t, shared.EndpointType, &envoy_discovery_v3.DeltaDiscoveryRequest{
		...
		envoy.SendDeltaReqACK(t, shared.ClusterType, 1)
		...
		envoy.SendDeltaReq(t, shared.ListenerType, nil)

		// It also (in parallel) issues the endpoint ACK
		envoy.SendDeltaReqACK(t, shared.EndpointType, 2)

		// And should get a response immediately.
		assertDeltaResponseSent(t, envoy.deltaStream.sendCh, &envoy_discovery_v3.DeltaDiscoveryResponse{
			TypeUrl: shared.ListenerType,
			Nonce:   hexString(3),
			Resources: makeTestResources(t,
		...
		// ACKs the listener
		envoy.SendDeltaReqACK(t, shared.ListenerType, 3)
		...
		envoy.SendDeltaReqACK(t, shared.EndpointType, 4)
	...
	runStep(t, "restore endpoint subscription", func(t *testing.T) {
		...
		envoy.SendDeltaReqACK(t, shared.EndpointType, 5)
	...
	runStep(t, "simulate envoy NACKing an endpoint update", func(t *testing.T) {
		...
		envoy.SendDeltaReqNACK(t, shared.EndpointType, 6, &rpcstatus.Status{})
		...
		envoy.SendDeltaReqACK(t, shared.EndpointType, 7)
...
func TestServer_DeltaAggregatedResources_v3_BasicProtocol_HTTP2(t *testing.T) {
	...
	envoy.SendDeltaReq(t, shared.ClusterType, nil)
	...
	runStep(t, "with-rds", func(t *testing.T) {
		...
		envoy.SendDeltaReq(t, shared.RouteType, &envoy_discovery_v3.DeltaDiscoveryRequest{
			ResourceNamesSubscribe: []string{
				"db",
			},
		})
		...
		// ACKs the listener
		envoy.SendDeltaReqACK(t, shared.ListenerType, 4)

		// And should get a response immediately.
		assertDeltaResponseSent(t, envoy.deltaStream.sendCh, &envoy_discovery_v3.DeltaDiscoveryResponse{
			TypeUrl: shared.RouteType,
			Nonce:   hexString(5),
			Resources: makeTestResources(t,
		...
		envoy.SendDeltaReqACK(t, shared.RouteType, 5)
...
func TestServer_DeltaAggregatedResources_v3_SlowEndpointPopulation(t *testing.T) {
	...
	server.ResourceMapMutateFn = func(resourceMap *shared.IndexedResources) {
		if atomic.LoadUint32(&slowHackDisabled) == 1 {
			return
		}
		if em, ok := resourceMap.Index[shared.EndpointType]; ok {
			for k := range em {
...
func TestServer_DeltaAggregatedResources_v3_BasicProtocol_TCP_clusterChangesImpactEndpoints(t *testing.T) {
	...
	runStep(t, "trigger cluster update needing implicit endpoint replacements", func(t *testing.T) {
		...
		envoy.SendDeltaReqACK(t, shared.ClusterType, 4)
...
func TestServer_DeltaAggregatedResources_v3_BasicProtocol_HTTP2_RDS_listenerChangesImpactRoutes(t *testing.T) {
	...
	runStep(t, "get into initial state", func(t *testing.T) {
		...
		envoy.SendDeltaReqACK(t, shared.RouteType, 4)
	...
	runStep(t, "trigger listener update needing implicit route replacements", func(t *testing.T) {
		...
		envoy.SendDeltaReqACK(t, shared.ClusterType, 5)

		// ACKs the listener
		envoy.SendDeltaReqACK(t, shared.ListenerType, 6)
		...
		envoy.SendDeltaReqACK(t, shared.RouteType, 8)
